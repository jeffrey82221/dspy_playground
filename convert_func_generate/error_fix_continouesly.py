...
def get_data_for_debug(func_string: str, input_values, target_values):
    __input_values = copy.copy(input_values)
    __target_values = copy.copy(target_values)
    global func
    exec(func_string, globals())
    __output_values = list(map(func, __input_values))
    data_for_debug = dict(map(lambda x: (x[0], {'correct_target_value': x[1], 'incorrect_output_value': x[2]}), 
                              filter(lambda x: x[1] != x[2], 
                            zip(__input_values, __target_values, __output_values))))
    return data_for_debug
...
iteration_cnt = 0
while accuracy < 1.0:
    print('[flow] iteration:', iteration_cnt, '................................................................')
    ...
    print('[flow] explaination:', explaination.code_explaination)
    ...
    response = revise(
        ...
        input_data_type=type(train_input_values[0]),
        target_data_type=type(train_target_values[0]),
        data_for_debug=get_data_for_debug(incorrect_function, train_input_values, train_target_values)
    )
    func_string = response.convertion_code
    func_string = func_string.replace('```python', '').replace('```', '')
    print('[flow] function string:')
    print(func_string)
    print('[flow] reasoning of how to fix the function:', response.reasoning)
    func = None
    exec(func_string, globals())
    ...
    print('[flow] accurecy:', accuracy)
    iteration_cnt += 1
    incorrect_function = func_string